...
<body>
    ...
    <form action="cargar-mesas.php" method="POST">
        ...
        <label id="turno"> Turno
        <select class="desplegable" id="turno" name="turno">
            <option disabled selected value=""> selecione aquí</option>
            <?php
            include "conexion.php";
            ...
            $row = mysqli_num_rows($ejecucion);
            if ($row > 0) {
                while ($turno = mysqli_fetch_assoc($ejecucion)) {
                    echo "<option value= " . $turno['id_tueno'] . "> " . $turno['turno'] . " </option>";
                }
            } else {
                echo "<option value=''>Aún no hay turnos escolares cargados.</option>";
            }
            ?>
        </select>
